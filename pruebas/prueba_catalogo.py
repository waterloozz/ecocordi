...
ok(config == {"whatsapp": None}, "Sin WHATSAPP_NUMERO: whatsapp = null (el botón se oculta)", config)
...
_, config = srv.cliente().pedir("GET", "/api/config")
ok(config == {"whatsapp": None}, "Un WHATSAPP_NUMERO inválido se ignora", config)
srv.borrar()
